...
- When Kanban runs on a headless remote Linux instance (for example over SSH+tunnel), native folder picker commands may be unavailable (`zenity`/`kdialog`). Treat this as a normal remote-runtime limitation and use manual path entry fallback instead of requiring desktop packages. Inside the Electron shell this does not apply: `window.desktop.dialogs.pickDirectory()` uses Electron's own dialog, which is always present.
- A desktop change is not done until you have built a packaged artifact and launched it. `npm --prefix packages/desktop test` and `typecheck` both pass on code that crashes a real build. Three such defects landed this way: an ESM/CJS import crash (below), a `deb` target that fails without an `author` email for fpm's maintainer field, and a Linux executable named `@kanbandesktop` because electron-builder derives it from the scoped npm package name (fixed with `linux.executableName`). On Linux you can do the whole loop headlessly: `npm run build` at root, then `npm run stage:cli && npm run build:ts && npx electron-builder --linux dir` in `packages/desktop`, then `xvfb-run -a ./out/linux-unpacked/kanban --no-sandbox --enable-logging`.
- **CommonJS deps in the Electron main process need default imports.** `packages/desktop` compiles to ESM, and a CJS package whose exports are defined via `Object.defineProperty` (as `electron-updater` does for `autoUpdater`) is invisible to `cjs-module-lexer`. A named import typechecks, passes every unit test, and then throws `does not provide an export named 'X'` the first time the *packaged* app loads it. Use `import pkg from "..."` and read the property inside a function — lazily, since these getters often construct something on first access. See the comment block in `src/updater/electron-updater-backend.ts`.
- macOS notarization is wired through `packages/desktop/scripts/notarize.cjs`, which reads **`APPLE_ID_PASSWORD`** (not `APPLE_APP_SPECIFIC_PASSWORD`) and **skips silently, returning success, when any credential is missing**. A release run therefore goes green and ships a DMG that Gatekeeper refuses to open. `desktop-release.yml` now fails a publishing macOS run up front if the credentials are absent — keep that guard.
- The root `npm run typecheck` covers neither `web-ui` nor `packages/desktop`. The pre-commit hook now runs all three, but if you typecheck by hand, run `npm --prefix web-ui run typecheck` and `npm --prefix packages/desktop run typecheck` too — CI does.
- Anything the desktop shell exposes to the web UI goes through `packages/desktop/src/bridge/contract.ts`, imported by `web-ui` via the `@desktop-bridge` alias (the same pattern as `@runtime-*`). Adding a capability means touching the contract, `ipc-schemas.ts`, `main-bridge.ts`, `preload.ts`, `main.ts`, and the renderer's `desktop-bridge.ts` — the renderer deliberately treats a capability as absent unless its method is actually callable, so a half-wired namespace degrades to browser mode rather than throwing.
